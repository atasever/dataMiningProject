...
allHelixPatternReader = open('data/helixChemical.txt', 'r+')
allLoopPatternReader = open('data/loopChemical.txt', 'r+')
allBetaPatternReader = open('data/betaChemical.txt', 'r+')
...
helixReader = open('results/helix_chemical_apriori_small_set_permutations.txt', 'r+')
loopReader = open('results/loop_chemical_apriori_small_set_permutations.txt', 'r+')
betaReader = open('results/beta_chemical_apriori_small_set_permutations.txt', 'r+')
...
for pattern in onlyHelix:
    ...
    for p in allBetaPatterns:
        if isInString(p,pattern):
            onlyHelixSubstracted.add(pattern)
print "total Beta : " ,len(beta)
print "total helix : ",len(helix)
print "total loop : ",len(loop)
